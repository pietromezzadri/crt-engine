...
class Menu:
    def __init__(
        self, _id, width, height, menu_items, renderer, input_handler, font, game_state
    ):
        self.id = _id
        self.width = width
        self.height = height
...
class OldMenu:
    def __init__(
        self, width, height, menu_items, renderer, input_handler, font, game_state
    ):
        self._id = 0
        self.width = width
        self.height = height
    ...
    def run(self):
        if actions.MAIN_GAME["DOWN"] in self.input_handler.keys_pressed:
            ...
            if self.selected >= 3:
                self.selected = 0
            self.input_handler.keys_pressed.remove(actions.MAIN_GAME["DOWN"])

        if actions.MAIN_GAME["UP"] in self.input_handler.keys_pressed:
            self.selected -= 1
            if self.selected < 0:
                self.selected = 2
            self.input_handler.keys_pressed.remove(actions.MAIN_GAME["UP"])

        if (
            actions.MAIN_GAME["ENTER"] in self.input_handler.keys_pressed
            ...
            if actions.MAIN_GAME["ENTER"] in self.input_handler.keys_pressed:
                self.input_handler.keys_pressed.remove(actions.MAIN_GAME["ENTER"])
        ...
        for index, text in enumerate(self.menu_items):
            ...
            if (
                self.input_handler.mouse.get_x() >= text_obj_x
    ...
    def options(self):
        ...
        menu_text = (
            f"Current Resolution - [{self.renderer.width}]"
