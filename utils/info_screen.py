...
class InfoScreen:
    ...
    def display_info(self, fps):
        left_up_corner = self.font.render_text(
                f"({self.renderer.x_start:.1f}, {self.renderer.y_start:.1f})", 'main', (150, 50, 50))
        right_up_corner = self.font.render_text(
                f"({self.renderer.x_end:.1f}, {self.renderer.y_start:.1f})", 'main', (150, 50, 50))
        left_down_corner = self.font.render_text(
                f"({self.renderer.x_start:.1f}, {self.renderer.y_end:.1f})", 'main', (150, 50, 50))
        right_down_corner = self.font.render_text(
                f"({self.renderer.x_end:.1f}, {self.renderer.y_end:.1f})", 'main', (150, 50, 50))

        self.renderer.render_to_screen(left_up_corner, 10, 10)
        self.renderer.render_to_screen(
            right_up_corner, self.renderer.width - 230, 10)
        self.renderer.render_to_screen(
